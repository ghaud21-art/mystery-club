...
.login-card{width:100%;max-width:420px;background:#FFFFFF;border-radius:20px;padding:44px 38px;border:1px solid #E5D9C8;box-shadow:0 8px 40px rgba(92,60,24,.14)}
...
// ── API 설정 ──────────────────────────────────────
const API_URL = "https://script.google.com/macros/s/AKfycbzqcK1R7_Y9FQrOe_B9fJv-ZbrfPzglnv9gty3C1Fbp2aAmC8GIaJEvEok_mdvFEv1-/exec"; // ← 배포 후 교체!
...
export default function App(){
  ...
  const loadScenarios = async () => {
    ...
    try {
      ...
      if(data && Array.isArray(data)) setScens(data);
    } catch(e) { console.log("시나리오 로컬 모드"); }
  ...
  const loadMembers = async () => {
    ...
    try {
      ...
      if(data && Array.isArray(data) && data.length > 0) setMbs(data);
    } catch(e) { console.log("멤버 로컬 모드"); }
  };
  ...
  return (
    <div className="login-wrap">
      <div className="login-card">
        ...
        <div style={{display:"flex",flexDirection:"column",gap:13,marginBottom:6}}>
          <Fld label="아이디">
            <input value={id} onChange={e=>setId(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()}
              ...
              style={{...inp,borderColor:err?T.red:T.border}} />
          </Fld>
          <Fld label="비밀번호">
        ...
        {err&&<div style={{fontSize:12,color:T.red,fontWeight:600,textAlign:"center",marginBottom:10,marginTop:4}}>{err}</div>}
        ...
        <button className="btn" onClick={onSignup} style={{width:"100%",padding:"10px",background:"transparent",color:T.gold,borderRadius:10,fontSize:13,border:`1.5px solid ${T.gold}`,fontWeight:700}}>+ 새 계정 만들기</button>
      </div>
    </div>
  );
}
...
function Signup({onDone,onBack,ids}){
  ...
  const finish=async ()=>{
    ...
    if(!API_URL.includes("여기에")){
      try { await apiPost("addMember", newMember); } catch(e){ console.log("멤버 저장 실패:", e); }
    }
    onDone(newMember);
  };
