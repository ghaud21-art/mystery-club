...
// ── API 설정 ──────────────────────────────────────
const API_URL = "https://script.google.com/macros/s/AKfycbx18zVZX9rKJtfBlUgvwFCXkIr-fPF4AUQrtuHh5S4w4rJ7BvqOlpQxyzOagBCeH2xO/exec"; // ← 배포 후 교체!
...
export default function App(){
  ...
  const loadMembers = async () => {
    ...
    finally { setMbsLoading(false); }
  };

  // ★ 앱 시작 시 자동으로 멤버 목록 불러오기
  // 다른 기기에서도 구글 시트의 최신 멤버로 로그인 가능
  useEffect(()=>{
    loadMembers();
  },[]);
    ...
    if(!API_URL.includes("여기에")){
      try{
        ...
        if(freshMbs&&freshMbs.length>0){
          ...
          allMbs=parsed;
          onRefreshMbs(parsed);
          console.log("✅ 로그인용 멤버 로드:", parsed.length, "명", parsed.map(m=>m.name));
        }
      }catch(e){ console.error("❌ 멤버 로드 실패:", e.message); }
    }
    const found=allMbs.find(m=>String(m.name).trim()===id.trim());
    if(!found){
      console.log("❌ 못찾음. 전체목록:", allMbs.map(m=>m.name));
      setErr("존재하지 않는 아이디입니다.");
      setTimeout(()=>setErr(""),2000);return;
    }
    if(String(found.pw).trim()!==String(pw).trim()){
      console.log("❌ 비번불일치. 저장된pw:", found.pw, "입력pw:", pw);
      setErr("비밀번호가 틀렸습니다.");
      setTimeout(()=>setErr(""),2000);return;
    }
    setErr("");
